...
def get_gnacenote_url(client_id):
	"""
...
def fetch_basic_tags_from_gracenote(clientID, userID, GNID):
	"""
	...
	# query_xml = get_gracenote_album_details_query(album_id, user_id, client_id)
...
# def get_empty_gracenote_metadata_struct():
# 	return {
# 		'xid': '',
# 		'mood': {},
# 	}
...
def register(client_id='673170061-74A1301CD818F3A0E54DD98D589B9186'):
	"""
	...
	query.addQueryClient(client_id)
	
	queryXML = query.toString()
	
	# POST query
	response = urllib_request.urlopen(_gnurl(client_id), queryXML)
	...
	if responseElem.attrib['STATUS'] == 'OK':
		...
		user_id = userElem.text
	
	return user_id


def search(artist,track,client_id='673170061-74A1301CD818F3A0E54DD98D589B9186', user_id=register(), album='', toc=''):
	"""
	...
	query.addAuth(client_id, user_id)
...
def getting_tags(mp3fileslist,tag):
	...
	for mp3dicts in mp3fileslist:
		...
		try:
			...
			try:
				_extract_tags(mp3dicts, supermetadict)
...
def _extract_tags(mp3dicts, supermetadict):
...
def sort_folder_mp3files(mp3file,user_enter,tag):
	...
	# for symbol_to_replace in '<>?:"/':
	# 	value = value.replace(symbol_to_replace, ' ')
	# for symbol_to_remove in '\x00^':
	# 	value = value.replace(symbol_to_replace, '')
